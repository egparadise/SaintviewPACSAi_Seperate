...
## 5. 영상 취득 모드 — Live / 다운로드 (설정 > 환경)

Live 모드에서 영상을 **미리 받아 두는** 선택지. `설정 > 환경 > 영상 취득` 에서 고른다.

| 모드 | 뜻 | 기본 |
|---|---|---|
| **Live** | 볼 때 받는다(기존 동작 그대로) | ✅ |
| **다운로드** | 워크리스트를 최근 환자 순으로 훑어 미리 받아 둔다 | |

> ⚠ 이름 주의 — `sv_server_mode`(local/web/live)와 연동 3가지 모드(미러/Live/표준 DICOM)와 별개다.
> 이것은 **영상 취득 모드**이고 저장 키는 `viewer.prefs.dl_*` 이다.

### 저장 위치·구조

- **OPFS**(Origin Private File System) — 권한 프롬프트 없음, 탐색기에 보이지 않음.
- 경로: `v1/{검사일}/{환자ID}__{StudyUID 끝12}/{Modality}/{Thumbnail|DICOM}/{sop}.{jpg|png}`
  - 폴더명 `DICOM` 은 사용자 확정 구조를 그대로 계승한 이름이고, **내용물은 서버 렌더 이미지**다
    (형식은 관리자 설정 — 아래 '영상 형식' 참조. 확장자가 곧 형식이다).
  - `__{StudyUID 끝12}` 는 **같은 날·같은 환자·같은 모달리티 검사 2건**의 충돌을 막기 위한 것이다
    (63cdafc 에서 반출 ZIP·폴더·ISO 를 망가뜨린 그 충돌). 회귀 테스트: `frontend/tests/dl_path_rule.test.mjs`
    (+ pytest 래퍼 `backend/tests/test_dl_mode_rules.py`).
- 캐시 키: `live|full|{형식태그}|{sop}` · `live|th|{sop}`
  - `live|` = **소스 구분자**. 미러 배치에서는 로컬 Orthanc 사본의 SOP UID 가 같으므로, 소스를
    키에 넣지 않으면 나중에 로컬로 범위를 넓혔을 때 A 판본과 로컬 판본이 같은 칸을 다툰다.
  - `{형식태그}`(`png`/`j90`/`j100`) = 관리자가 형식을 바꾸면 이미 받아 둔 바이트는 다른 형식이므로
    키가 갈려야 한다(안 그러면 PNG 로 바꿔도 옛 JPEG 이 계속 히트한다).

### 영상 형식 — 관리자 설정을 따른다

저장본 URL 은 `imageFormat.fixedRenderedParams()` 로 만든다: **관리자 설정(`viewer.image_format.h{hid}`)
의 png/jpeg·quality 를 그대로 따르고**, 회선 상태에 따라 결과가 바뀌는 자동 판정(`autoJpegForSlowLink`)
만 배제한다(같은 이미지가 회선별로 다른 바이트가 되면 캐시가 갈린다). `default` 는 서버 기본과 같은
JPEG q90 으로 고정한다.

> 예전에는 `accept=image/jpeg&quality=90` 을 하드코딩해서, **무손실 PNG 로 판독하도록 설정한 병원에서
> 다운로드 모드를 켜는 순간 화면이 아무 표시 없이 손실 JPEG 으로 강등**됐다. 프론트가 관리자 정책을
> 우회하는 유일한 경로였다. 워크리스트도 뷰어와 같은 시점에 `api.hospImageFormat(hid) → setImageFormat`
> 을 호출해 스케줄러가 옳은 형식을 알고 시작하게 한다.
- 원자성: 파일 기록 → **IndexedDB 인덱스(`sv_dl`) 커밋** 순서. 조회는 항상 인덱스 → OPFS 이므로
  인덱스에 없는(=쓰다 만) 파일은 구조적으로 읽히지 않는다.

### 동작 범위 (1회차)

- **Live 모드 + 2D 뷰어 3종**(SaintView·T-View = `Viewer2D`, I-View = `ViewerInfi`)에서만.
- 3D 뷰어·로컬 Orthanc 모드·오프라인 워크리스트(메타 캐시)는 범위 밖.
- 이 범위는 **쓰기·읽기 양쪽에 게이트**로 박혀 있다: 스케줄러는 `liveMode` 에서만 돌고,
  뷰어도 `isLiveStudyUid(stu)` 인 페인에서만 저장본을 꺼낸다. 읽기 게이트가 없으면 미러 배치에서
  `sv_server_mode` 를 local 로 바꾼 순간 로컬 검사 화면에 A 가 렌더한 저장본이 뜬다(페인 단위 판정이라
  Combine 으로 Live·로컬을 섞어도 각 인스턴스가 제 소스로 간다).
- 다음 조건에서는 **자동으로 비활성**되고 설정 화면에 사유가 뜬다:
  비보안 컨텍스트(http) · OPFS/IndexedDB 미지원 · `VITE_VIEWER_BASE` 배치(뷰어가 다른 오리진이면 저장소를 공유하지 못한다).

### 안전장치

- **W/L 오염 방지** — 저장본은 검사 기본 W/L 로 구운 JPEG 이라, 사용자가 W/L 을 조정 중이면(`p.wl` 비어 있지 않음)
  캐시를 쓰지 않고 서버 렌더로 폴백한다.
- **낡은 영상 방지** — A SSE `changed_studies` 를 워크리스트가 구독해 해당 검사 저장본을 폐기한다(`dlInvalidate`).
  - ID 공간을 맞춘다: `changed_studies` 는 A 원본 **study_idx** 이고 큐의 `studyId` 는 **vid = VID_BASE + idx** 다.
    그대로 넘기면 매칭이 구조적으로 절대 성립하지 않아 무효화가 영구 무동작이 된다(실제로 그 상태였다).
  - 백엔드 목록은 누적(`[-200:]`)이라 **새로 늘어난 것만** 넘긴다(`freshChangedVids`) — 통째로 넘기면
    rev 가 오를 때마다 200건을 다시 폐기하는 '폐기·재다운로드 폭주' 가 된다.
  - 폐기는 **창을 넘는다**: 뷰어는 별도 창(window.open)이라 모듈 상태가 따로 논다. 파일만 지우면 뷰어의
    blob URL 캐시가 낡은 영상을 계속 반환하므로, `BroadcastChannel`(`lib/sync.ts` `dl-invalidate`)로
    모든 창의 캐시를 비우고 재조회시킨다(설정 '지금 비우기' 도 같은 신호를 쓴다).
- **프라이버시** — 로그아웃·세션 만료·계정 전환 시 `opfsWipe()` 로 **전부 삭제**한다
  (`api.setToken(null)` 경로 + 워크리스트 Logout 버튼). 공용 판독 PC 전제.
  - 순서가 규칙이다: **다운로더 정지(`sv-auth-cleared` → `dlReset`) → 삭제 → (완료 후) 리로드**.
    401 경로가 삭제를 기다리지 않고 리로드하면 삭제가 중간에 끊겨 파일이 남는다(최대 3초 대기 후 리로드).
    `opfsWipe` 자체도 **파일 먼저·인덱스 나중** 이라 중간에 끊겨도 '인덱스만 남음'(조회 시 자가 정리)
    쪽으로 실패한다. 정지 뒤에도 남는 in-flight 레이스는 `opfsPut` 의 토큰 가드가 닫는다.
- **서버 부하** — 동시 받기 기본 2(최대 4). 검사를 여는 순간 그 검사를 큐 맨 앞으로 올리고 몇 초 감속한다.
- **중복 방지** — Web Locks(`sv-dl-scheduler`)로 창이 여럿이어도 **한 창만** 다운로더가 된다.
- **용량** — `min(설정 상한, 브라우저 할당량 × 0.5)`. 초과 시 **검사 단위 LRU**(오래 안 본 검사부터 통째로).

### 수동 검증 체크리스트

> ⚠ 프론트에는 브라우저 런타임 테스트 프레임워크가 없다. OPFS·Web Locks·decode 타이밍은
> `tsc -b` / `npm run build` 로 한 줄도 검증되지 않는다. 아래는 **사람이 직접** 밟아야 한다.

1. 다운로드 모드로 저장 → 워크리스트 방치 → DevTools ▸ Application ▸ Storage ▸ File System 에 `v1/...` 생성 확인.
2. 받아 둔 검사 열기 → Network 탭에 `rendered` 요청이 나가지 않는지.
3. 시리즈 스크롤 중 깜빡임(디코드 스톨) 없는지 — 전환이 무음인지 육안 확인.
4. W/L 드래그 → Network 에 `rendered?window=...` 가 다시 나가는지(= 캐시 폴백 규칙 동작).
5. 상한을 작게(1GB) 두고 방치 → 오래된 검사 폴더부터 사라지는지.
6. 로그아웃 → Application ▸ File System / IndexedDB(`sv_dl`) 가 비었는지.
7. 워크리스트 창을 2개 띄우고 설정 ▸ 환경 ▸ 진행 표시를 비교 → **한 창만** '받는 중' 인지.
8. A 에서 검사 픽셀 교체 → SSE 반영 후 그 검사 저장본이 폐기되고 다시 받아지는지.
   **뷰어 창을 열어 둔 채로** 확인할 것 — 창 간 무효화가 죽으면 뷰어만 낡은 영상을 계속 띄운다.
9. 관리자 ▸ 병원 ▸ 영상 전송 형식을 PNG(무손실)로 바꾸고 다시 받기 → 저장 파일이 `.png` 인지,
   뷰어가 그 PNG 를 쓰는지(Network 에 `rendered` 가 안 나가는지).
10. 설정 ▸ 환경 ▸ [지금 비우기] → 사용량 0 이 된 뒤 **다시 받기 시작**하는지(진행률이 0/N 부터 다시).
11. 세션 만료(토큰 폐기) → 리로드 후 Application ▸ File System 이 비어 있는지.
