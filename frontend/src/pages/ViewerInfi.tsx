...
export function ViewerInfi({ detail, onClose, addDetail, stackDetail, keySops, withOpen, navFilter }: {
  ...
  const dict = useDictation((text) => {
    ...
    say(`🎙 ${tr("음성 판독 → 판독문 삽입")}: “${text.slice(0, 30)}${text.length > 30 ? "…" : ""}”`);
  });
  ...
  useEffect(() => {
    ...
    // Compare 로 재진입한 경우 — Sync With Other Exams 자동 ON + M/S 라벨 활성 (플래그는 아래 행잉 판단에도 사용)
    const cmpRe = localStorage.getItem("sv_infi_cmp") === "1";
    if (cmpRe) {
    ...
    Promise.all(ids.map(async (id) => {
      ...
      // 로드에 실패한 id 는 레지스트리에서도 제거 — 죽은 검사가 창마다 영구 재시도되는 것 방지
      if (list.length !== ids.length) {
        const ok = new Set(list.map((e) => e.d.id));
        try { localStorage.setItem(EXAMS_KEY, JSON.stringify(ids.filter((id) => ok.has(id)))); } catch { /* 무시 */ }
      }
      ...
      // 다중 모니터 관리 배치(mm — openV2 URL 승격 + 창 세션 영속)는 공유 레지스트리 전체를 탭으로
      // 유지해야 하므로 필터를 건너뛴다: 모든 모니터가 동일한 Exam 탭(연 순서), 활성 검사만 창마다 다름.
      const mm = mmManaged();
      const explicit = !!(addDetail || stackDetail || withOpen || keySops?.length) || mm;
      if (!explicit) {
      ...
      // mm(다중 모니터 관리 배치): 레지스트리 전체는 'Exam 탭'으로만 유지 — 페인 행잉은 이 창의
      // 배정 검사(+명시적 다중 오픈 대상: Add/Stack/With Open)만. 아니면(단일 창·Compare 재진입) 기존대로
      // 전체 누적 행잉. 이 분리가 없으면 mm 창마다 레지스트리 전 검사(타 환자 포함)가 그리드로 깔린다.
      const wids = new Set(withOpen?.ids ?? []);
      // (Stack View 는 위에서 early-return — 여기 도달 시 stackDetail 은 항상 없음)
      const hangList = (mm && !cmpRe)
        ? list.filter((e) => e.d.id === detail.id || e.d.id === addDetail?.id || wids.has(e.d.id))
        : list;
      ...
      // 활성 페인 — 행잉된 페인 중 배정 검사의 위치(mm 단독 행잉이면 0. 비-mm 은 기존 ai 와 동일 결과)
      setActive(Math.max(0, hangList.findIndex((e) => e.d.id === detail.id)));
    }).catch(() => {});
  }, [detail]);
  ...
  const proceedCloseExam = (i: number) => {
    ...
    if (!remain.length) {
      // 마지막 검사 탭을 닫으면 이 창은 **진짜로** 닫힌다(리로드 아님) → 슬롯 즉시 반납.
      // ⚠ 여기만 releaseViewerSlot 이 빠져 있어 proceedCloseAll(810)·Viewer2D.doClose 와 비대칭이었다.
      //   반납이 없으면 워크리스트가 '창이 살아 있다'로 보는 시간이 pagehide 유예(8초)만큼 길어져,
      //   다운로드 정지·기준선 해제가 그만큼 늦고 다음 오픈의 ①부트스트랩 판정도 흔들린다.
      // 창이 통째로 닫히므로 'All Close' 와 같은 뒷정리를 한다 — 전파 자체는 doCloseAction 이 확정
      // 시점에 이미 방송했다(requestClose 의 decideCloseScope). 벽이 비면 장부·순번도 초기화해야
      // 다음 오픈이 ①부트스트랩으로 시작한다.
      if (shouldCloseAllMonitors(closeScopeRef.current)) clearViewerSlots();
  ...
  const proceedCloseAll = () => {
    ...
    // ⚠ 판정은 lib/viewerClose.shouldCloseAllMonitors 한 곳 — Viewer2D 와 갈리지 않게(예전엔
    //    두 파일에 서로 다른 저장소(state/ref)로 두 번 하드코딩돼 있었다).
    if (shouldCloseAllMonitors(closeScopeRef.current)) {
  ...
  const requestClose = (req: CloseReq) => {
    // 전파 판정은 lib/viewerClose.decideCloseScope 한 곳. **'모든 검사 닫기' 버튼만이 아니다** —
    // Exam 탭 ✕ / ⊠Close▸'현재 검사 닫기' 로 **마지막** 검사를 닫아도 이 창은 통째로 사라지므로
    // 같은 규약을 태워야 한다(예전엔 그 경로에 전파가 없어 다른 모니터가 그대로 남았다).
    // 전파 자체는 요청 시점이 아니라 확정 시점(doCloseAction)에 — ask 다이얼로그를 취소하면
    // 다른 모니터만 닫혀 있는 비원자 상태가 되기 때문.
    const d = decideCloseScope(req.kind === "all" ? "all_close" : "one_exam",
                               req.kind === "one" ? exams.length - 1 : 0,
                               closeScopeRef.current);
    if (d.allMonitors) closeAllBroadcastRef.current = true;
  ...
  useEffect(() => onViewerCloseAll(() => {
    // 저장(await) **전에** 슬롯부터 반납 — 남겨 두면 2초 하트비트가 발신 창이 지운 sv_vslot_* 를
    // 되살려, All Close 직후의 다음 오픈이 ①부트스트랩 대신 라운드로빈으로 떨어진다.
    releaseViewerSlot(); releaseHeldStudies();
  ...
  // ViewerWindow(뷰어 껍데기)의 All Close 안전망이 '뷰어가 떠 있으니 저장을 기다린다'를 판단하는 근거.
  ...
  const cmpMasterUid = cmpMaster || detail.study_uid;
  const cmpSlaveUids = useMemo(() => {
    const out: string[] = [];
    for (const p of panes) {
      if (p.studyUid && p.studyUid !== cmpMasterUid && !out.includes(p.studyUid)) out.push(p.studyUid);
    }
    return out;
  }, [panes, cmpMasterUid]);
  const paneCmpRole = (uid?: string): string | null => {
    if (!cmpLabelsOn || !cmpActive) return null;        // ⇄ Compare 명시 진입 시에만(Add/Stack/드래그 제외)
    if (uid === cmpMasterUid) return cmpRole || "M";    // 기준 검사 — master="M", slave 창=URL cmprole
  ...
  const applyHpCells = (rule: HpRule) => {
    ...
    if (none.length) say(`${none.join("·")}${tr("번 칸: 조건에 맞는 과거검사가 없습니다")}`);
    else if (d3.length) say(`${d3.join("·")}${tr("번 칸: 3D 영상은 뷰어가 복원하지 않습니다")}`);
  };
  ...
  useEffect(() => {
    const cur = () => examsRef.current[activeExamRef.current]?.d.id ?? detail.id;
    const off = startViewerSlotHeartbeat(cur);
    // ★ 다운로드 모드 기준선의 **상승 에지** 저지연 트리거(lib/sync.postViewerOpened 주석).
    //   판정은 워크리스트의 슬롯 장부 폴링이 하므로 이 방송이 유실돼도 무해하다.
    if (isViewerSlotName(window.name)) postViewerOpened(window.name, cur());
    return off;
  }, [detail.id]);
  // 다운로드 모드 축출 보호 장부 — "이 창이 지금 물고 있는 **모든** Exam 탭의 StudyUID".
  // ★ In-View 는 예전에 이 보호에서 통째로 빠져 있었다: 보호 출처가 Viewer2D 전용 장부
  //   (sv_viewer_tabs)였고, In-View 자기 장부(sv_infi_exams)는 uid 없는 id 배열이라 읽을 수도
  //   없었다. 그래서 단일 모니터 + In-View 조합에서는 '보고 있는 검사는 삭제하지 않습니다' 라는
  //   설정 화면의 약속이 성립하지 않았다. 두 뷰어가 같은 TTL 장부(lib/dlHeld)에 uid 를 직접 남긴다.
  const heldUidsRef = useRef<string[]>([]);
  heldUidsRef.current = [...new Set([
    detail.study_uid, ...exams.map((e) => e.d.study_uid),
  ...
  const saveAnnos = async (): Promise<void> => {
    ...
    say(`${tr("주석")} ${n}${tr("건 + 표시상태 저장됨 (서버) — 재오픈 시 재현")}${skipped ? ` · ${skipped}${tr("건 제외(과거 시리즈)")}` : ""}`);
  };
  ...
  const applyGsps = (it: GspsItem) => {
    ...
    setGspsPick(null);
    schedHist();
    say(`${tr("GSPS 적용")} — ${it.label || it.creator || "PR"} · ${tr("주석")} ${it.annotations.length}${tr("건 (녹색=외부)")}`);
  };
  ...
  const doCtrAi = () => {
    ...
    api.ctr(ex.d.id).then(async (r) => {
      ...
      schedHist();
    }).catch((e) => say(e instanceof Error ? e.message : tr("CTR 실패")));
  };
  ...
  const fire = (id: string) => {
    ...
    switch (id) {
      ...
      case "key2d": {   // F-16: 현재 이미지 키이미지 토글 → DB(study.key_images) 저장
        ...
        api.instances(exd.id).then((r) => {
          ...
          return api.setKeyImages(exd.id, next).then(() => {
            ...
            say(exists ? `🔑 ${tr("키이미지 해제 — 남은")} ${next.length}${tr("장")}`
                       : `🔑 ${tr("키이미지 등록")} (${next.length}${tr("장")}) — ${tr("워크리스트에 🔑 표시")}`);
          });
      ...
      case "dictation":   // 음성 판독(Rec) — 서버 STT 엔진으로 녹음/전사 → 판독문 삽입
        dict.toggle();
        say(dict.recording ? tr("🎙 녹음 정지 — 전사 중…")
      ...
      case "saveAnno":   // IN-1 ①: 주석 서버 저장 (계정 로밍)
        saveAnnos().catch((e) => say(e instanceof Error ? e.message : tr("주석 저장 실패")));
        break;
      case "gsps":       // IN-1 ③: 주석+W/L → Presentation State 저장
        doGsps().catch((e) => say(e instanceof Error ? e.message : tr("GSPS 저장 실패")));
        break;
      case "gspsLoad": { // IN-1 ③: 검사 귀속 PR 목록 → 선택 적용
        ...
        api.loadGsps(ex.d.id).then((r) => {
          ...
          setGspsPick(r.items);
        }).catch((e) => say(e instanceof Error ? e.message : tr("GSPS 조회 실패")));
        break;
      }
      ...
      case "shutRect": case "shutEl": case "shutPoly": {
        ...
        if (panes[active]?.shutter) {
          ...
          say(id === "shutPoly" ? tr("다각 셔터 — 점을 찍고 더블클릭으로 완료 (다시 누르면 해제)")
            : id === "shutEl" ? tr("타원 셔터 — 드래그로 영역 지정 (다시 누르면 해제)")
            : tr("사각 셔터 — 드래그로 영역 지정 (다시 누르면 해제)"));
        }
        break;
      }
      default: {
        ...
        if (["select", "pan", "zoom", "wl"].includes(id) || item?.mode) {
          if (tool === id && id !== "select") {
            ...
            setTool("select"); setPend(null);
            say(`${tr((item?.label ?? id).split("—")[0].trim())} ${tr("해제")}`);
            break;
          }
          setTool(id as Tool);
          setPend(null);
  ...
  const applyCalibrationInfi = (pi: number, pts: { x: number; y: number }[]) => {
    ...
    setPanes((ps) => ps.map((q, k) => (k === pi && q.series
      ...
      : q)));
    say(`✅ ${tr("Calibrate 완료")} — 1px = ${mmPerPx.toFixed(4)}mm (${tr("기준")} ${mm}mm / ${pixLen.toFixed(1)}px). ${tr("이후 측정은 mm 로 표시")}`);
  };
    ...
    switch (tool) {
      ...
      case "table2d": {
        void samplePixels(url, inst.cols || 1, inst.rows || 1).then((data) => {
          ...
          setTableData({ title: `2D Table ≈${tr("픽셀값")} (${x0},${y0})~(${x1},${y1}) step ${step}`, rows });
        });
        break;
      }
    }
  ...
  const deleteSelAnnos = (): boolean => {
    ...
    schedHist();
    say(`${tr("선택한")} ${sel.idxs.length}${tr("개 주석을 지웠습니다")}`);
    return true;
  };
  ...
  const endDrag = (e?: { type: string; clientX: number; clientY: number }) => {
    ...
    if (mq) {
      ...
      setMarquee(null);
      if (idxs.length) { setSelAnnos({ sop: mq.sop, idxs }); setSelAnno(null); say(`${idxs.length}${tr("개 주석 선택 — Del/휴지통/우클릭으로 삭제")}`); }
  ...
  const srsPageStep = (dir: number) => {
    if (srsPageMax <= 0) { setToast(tr("표시할 다음 시리즈가 없습니다")); return; }
    ...
    setToast(`${tr("시리즈")} ${pageLabel(pg, series.length, srsPageSize)}`);
  };
  ...
  const combineInto = (pi: number, seriesUid: string) => {
    ...
    setActive(pi);
    say(`Combine — Se${dropped.s.series_number} ${dropped.s.instances.length}${tr("장 추가 · 총")} ${merged.instances.length}${tr("장(연속 스크롤)")}`);
  };
  ...
  const switchExam = (i: number) => {
    ...
    if (pi >= 0) {
      ...
      setToast(tr("이 검사에서 보던 화면 구성을 복원했습니다"));
    } else {
      setMaximized(null);
  ...
  const dockLoadPrior = (examId: number) => {
    ...
    if (cmpCfgRef.current.prior_mode === "monitor"
      ...
      setCmpActive(true);                 // 이 창=기준(녹색 Compare M), 인접 모니터 창=S1
      setCmpMaster(curD.study_uid);       // 기준=비교를 시작한 활성 검사
      // 다른 열린 뷰어 창에도 과거검사 Exam 탭 동기 + 이 창(발신자 — BroadcastChannel 미수신)에도 추가
      postViewerAddTab(examId, re?.study_uid ?? "", `${re?.modality ?? "검사"} ${re?.study_date ?? ""} #${examId}`);
      if (!exams.some((x) => x.d.id === examId)) {
        void (async () => {
          try {
      ...
      say(`${tr("과거검사 비교")} — ${re ? `${re.study_date} ${re.modality}` : `#${examId}`} → ${tr("인접 모니터")}`);
      return;
    }
    api.seriesTree(examId).then((r) => {
      ...
      if (re) say(`${tr("과거검사 비교")} — ${re.study_date} ${re.modality} ${tr("(1:2 분할)")}`);
  ...
  useEffect(() => {
    api.getSetting("viewer.prefs").then((r) => {
      ...
      // 권한 미승인 등으로 실패하면 빈 캐시 유지 → 해당 기능은 Layout(인플레이스) 폴백.
      if (monScreensRef.current.length > 1) {
        void screenFeaturesList(monScreensRef.current)
  ...
  const dropSeriesToPaneInfi = (pi: number, seriesUid: string) => {
    ...
    if (cur) { upd(pi, { series: cur, index: 0, studyUid: curD.study_uid }); setActive(pi); say(`${tr("시리즈")} Se${cur.series_number} → ${tr("페인 로드 (드래그앤드롭)")}`); return; }
    const pr = priorSeries.find((e) => e.s.series_uid === seriesUid);
    if (pr) { upd(pi, { series: pr.s, index: 0, studyUid: pr.uid }); setActive(pi); say(`${tr("[과거]")} Se${pr.s.series_number} → ${tr("페인 로드 (드래그앤드롭)")}`); return; }
      ...
      <div data-pane={pi} onMouseDown={(e) => onPaneMouseDown(e, pi)} onMouseMove={onMouseMove}
          ...
          return (
            <div key={t} ref={sizeRef(`${pi}:${t}`)} data-sv-mg={mgStamp(mgf)}
                 ...
                 onDoubleClick={(e) => {
                   ...
                   setActive(pi);
                   setToast(`${tr("이미지")} ${idx + 1}/${insts.length} — ${tr("스크롤로 시리즈 전체 이동 (더블클릭: 분할 복귀)")}`);
                 }}
        ...
        {p.series && insts.length > 1 && (
          <div onMouseDown={(e) => e.stopPropagation()} onDoubleClick={(e) => e.stopPropagation()}
            ...
            <span title={p.playing ? tr("Pause — 이 페인 정지") : tr("Play — 이 페인만 재생")}
  ...
  const thumbCol = (
    <div ref={thumbColRef} style={{ width: ui.thumbW, background: "var(--bg-canvas)", borderRight: "1px solid var(--border)",
      ...
      <button onClick={() => {
              ...
              title={`${tr("썸네일 표시 모드 전환 (계정 로밍) — 현재")}: ${thumbMode === "series" ? tr("시리즈 대표 1장") : tr("전체 이미지 나열(최대 200장)")}`}
              style={{ fontSize: 10.5 }}>
        {thumbMode === "series" ? tr("시리즈") : tr("전체")}
      </button>
        ...
        .map(({ s, inst, idx }) => (
          <div key={inst.sop_uid} data-sop={inst.sop_uid} style={{ position: "relative", flexShrink: 0 }}>
            <img src={inst.preview_url} alt="" loading="lazy" decoding="async"
                 title={`S${s.series_number} Img${inst.instance_number ?? idx + 1}${keyMarks.has(inst.sop_uid) ? " · 🔑 KEY" : ""} — ${tr("클릭=활성 페인 표시")}`}
      ...
      {thumbMode === "series" && series.map((s, sIdx) => (
        <div key={s.series_uid} draggable data-suid={s.series_uid}
             ...
             title={`Se${s.series_number} · ${s.series_desc}\n· ${tr("드래그 → 원하는 페인에 놓으면 그 페인에 표시")}\n· ${tr("클릭: 활성 페인 (Ctrl=페인 선택 토글 · Shift=범위 선택)")}`}
      ...
      {priorSeries.length > 0 && (
        <div title={tr("불러온 과거검사 시리즈는 History 탭에서 관리합니다")}
             style={{ fontSize: 9, color: "#854d0e", textAlign: "center", padding: "2px 0" }}>
          {tr("과거")} {priorSeries.length}
        </div>
      )}
    </div>
  );
  ...
  const buildCtxItems = (): CtxItem[] => [
    ...
    { kind: "sep" },
    { label: `${tr("W/L 프리셋")} (${curD.modality})`, icon: "▸", children: wlPresets.map((p) => ({
      label: tr(p.label), onClick: () => applyWlInfi(p.q),
    })).concat([{ label: tr("반전(Invert)"), onClick: () => fire("invert") }]) },
    { label: tr("방향"), icon: "▸", children: [
      { label: tr("좌 90°"), onClick: () => fire("rotL") },
      { label: tr("우 90°"), onClick: () => fire("rotR") },
      { label: "180°", onClick: () => fire("rot180") },
      { label: tr("좌우 반전"), onClick: () => fire("flipH") },
      { label: tr("상하 반전"), onClick: () => fire("flipV") },
    ] },
    { label: tr("보기"), icon: "▸", children: [
      ...
      { label: maximized === active ? tr("최대화 해제") : tr("최대화"), onClick: () => setMaximized((m) => (m === active ? null : active)) },
    ] },
    { kind: "sep" },
    // 삭제(§C) — 마퀴 다중 > 단일 선택 > 마지막 + 전체 지우기(clrAnno)
    selAnnos
      ? { label: `${tr("선택")} ${selAnnos.idxs.length}${tr("개 주석 지우기")}`, onClick: () => deleteSelAnnos() }
      : selAnno
        ? { label: tr("선택 주석 지우기"), onClick: deleteSelAnno }
        : { label: tr("마지막 주석 지우기"), onClick: () => deleteLastAnno(active) },
    { label: tr("주석 전체 지우기"), onClick: () => fire("clrAnno") },
    // 셔터(가림) 해제 — 활성 페인 셔터 제거
    { label: tr("셔터 해제"), onClick: () => upd(active, { shutter: null }) },
    { kind: "sep" },
    { label: tr("Key Image 지정/해제"), onClick: () => fire("key2d") },
    { label: tr("주석 저장"), onClick: () => void saveAnnos() },
    { kind: "sep" },
    { label: tr("마우스 조작"), icon: "▸", children: IN_MOUSE_OPS.map((m) => ({
      label: `${tr(MOUSE_KEY_LABELS[m.key] ?? m.key)} · ${tr(m.action)}`, disabled: true,
    })) },
  ];
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%", background: "#000", color: "var(--text-secondary)" }}
        ...
        return (
          <div data-sv-rshield="" style={{ position: "fixed", inset: 0, zIndex: 300 }} onClick={() => setCircle(null)}
            ...
            <div onClick={(e) => e.stopPropagation()}
              ...
              {btn("Open", tr("이 페인에 표시(교체)"), () => dropSeriesToPaneInfi(c.pi, c.uid), true)}
              {btn("Combine", tr("현재 시리즈에 이어붙임"), () => combineInto(c.pi, c.uid))}
              {btn("Combine all", tr("검사 전체 시리즈 결합"), () => combineAllInto(c.pi))}
            </div>
          </div>
        );
      })()}
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "3px 10px",
        ...
        <span style={{ display: "flex", gap: 4, alignItems: "center", marginLeft: 10 }}>
          ...
          <button title={panes[active]?.playing
                    ? tr("Pause — 재생 정지")
                    : tr("Play — 자동으로 다음 영상 넘기기 (멀티 선택 시 선택 페인 모두)")}
                  onClick={() => fire("cine")}
                  style={{ marginLeft: 8, padding: "2px 12px", fontSize: 12, fontWeight: 700,
      ...
      <div style={{ display: "flex", flex: 1, minHeight: 0 }}>
        ...
        {!toolsOpen && (
          <button onClick={() => toggleTools(true)} title={tr(railSpec("left").showTitle)}
        ...
        <div style={{ width: ui.toolW, background: "var(--bg-panel)", borderRight: "1px solid var(--border)",
          ...
          <button title={tr(railSpec("left").hideTitle)} onClick={() => toggleTools(false)}
          ...
          <select title={tr("빠른 레이아웃 — 행잉 규칙과 무관한 즉석 분할")} defaultValue=""
                  onChange={(e) => {
            ...
            <option value="" disabled>{tr("빠른 레이아웃")}</option>
              ...
              <div style={{ position: "absolute", left: 0, top: "105%", zIndex: 30, background: "var(--bg-elevated)",
                ...
                {[["현재 검사 닫기", () => requestClose({ kind: "one", i: activeExam })],
                  ...
                  <div key={label as string} onClick={fn as () => void}
                       style={{ padding: "5px 8px", cursor: "pointer" }}>{tr(label as string)}</div>
                ))}
              </div>
            )}
          </span>
          ...
          <div style={{ overflowY: "auto", display: "flex", flexDirection: "column", gap: 1 }}>
            {(() => {
              const toolBtn = (t: (typeof PALETTE)[number]) => {
                ...
                return (
                  <button key={t.id} title={tr(t.label)} onClick={() => t.impl && fire(t.id)}
                    ...
                    {toolLabels && (
                      <span style={{ fontSize: 8.5, lineHeight: 1.1, textAlign: "center", width: "100%",
                                     ...
                                     WebkitBoxOrient: "vertical" }}>{tr(name)}</span>
                    )}
                  </button>
                );
              };
        ...
        {wlPanel && (
          <div style={{ width: ui.wlW, background: "var(--bg-panel)", borderLeft: "1px solid var(--border)",
            ...
            <div onClick={() => {
                   ...
                   if (!ai) {
                     say(real ? tr("AI 추천 W/L 미지원 — CT 검사에서 사용하세요")
                              : tr("보정 정보 없는 데이터 — AI W/L 미적용(서버 기본 유지)"));
                     return;
                   }
                   updMany(targetsOf(active), () => ({ wl: ai.q }));
                   schedHist();
                   say(`${tr("AI 추천 W/L 적용")}: ${tr(ai.label)} (${ai.q})`);
                 }}
              ...
              <button className="primary" disabled={!cmpSel.size}
                      onClick={() => {
                        ...
                        if (cmpCfg.multi_monitor !== false && placeCompareSlaves(cmpSlotsRef.current, window.name, sel)) {
                          setCmpActive(true);
                          setCmpRole("M");
                          setCmpMaster(curD.study_uid);
                          // 다른 열린 창 + 이 창(발신자)에도 비교검사 Exam 탭 동기
                          for (const id of sel) {
                            const rx = (curD.related_exams ?? []).find((q) => q.id === id);
                            postViewerAddTab(id, rx?.study_uid ?? "", `${rx?.modality ?? "검사"} ${rx?.study_date ?? ""} #${id}`);
                            if (!exams.some((x) => x.d.id === id)) {
                              void (async () => {
                                try {
                                  const d = await api.study(id);
                                  const t = await api.seriesTree(id);
        ...
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.6)", zIndex: 250,
          ...
          <div style={{ background: "var(--bg-panel)", border: "1px solid var(--border)", borderRadius: 8,
            ...
            <div style={{ fontSize: 10.5, color: "var(--text-secondary)", marginTop: 6 }}>
              {tr("렌더 영상 기반 근사값(W/L 역변환 ≈) — 원본 픽셀값 아님")}
            </div>
          </div>
        </div>
      )}
          ...
          <div style={{ background: "var(--bg-panel)", border: "1px solid var(--border)", borderRadius: 8,
            ...
            {gspsPick.map((it) => (
              <div key={it.sop_instance_uid} onClick={() => applyGsps(it)}
                ...
                <b>{it.label || tr("(라벨 없음)")}</b>
                <span style={{ color: "var(--text-secondary)", marginLeft: 8 }}>
                  {it.creator || "unknown"} · {tr("주석")} {it.annotations.length}{tr("건")}
      ...
      {closeDlg && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.6)", zIndex: 260,
                      display: "grid", placeItems: "center" }}
             /* 취소(배경 클릭)하면 전파 의도도 버린다 — 남기면 나중에 다른 모니터의 방송을 받아
                닫힐 때 이 창이 다시 방송해(수신 → 재전파) 창들이 서로 되받는다. */
             onMouseDown={(e) => {
               if (e.target !== e.currentTarget) return;
               closeAllBroadcastRef.current = false;
               setCloseDlg(null);
             }}>
          <div style={{ background: "var(--bg-panel)", border: "1px solid var(--border)", borderRadius: 8,
                        padding: 16, width: "min(420px, 92vw)" }}>
            <b style={{ fontSize: 13 }}>
              {closeDlg.kind === "all" ? tr("모든 검사 닫기") : tr("검사 닫기")} — {tr("변경사항 저장")}
            </b>
            ...
            {([["save_current", "주석 저장 후 닫기", "측정·주석을 서버에 저장(계정 로밍)"],
              ...
              <button key={m} className={m === "save_current" ? "primary" : ""}
                ...
                <b>{tr(label)}</b>
                <div style={{ fontSize: 10.5, color: "var(--text-secondary)", marginTop: 1 }}>{tr(desc)}</div>
              </button>
            ))}
            <label title={tr("체크하고 닫으면 다음부터 묻지 않고 이 동작으로 닫습니다 (viewer.prefs.infi_close_mode)")}
              ...
              {tr("이 선택을 기본 닫기 동작으로 저장(다음부터 묻지 않음)")}
            </label>
            <div style={{ textAlign: "right", marginTop: 8 }}>
              <button onClick={() => { closeAllBroadcastRef.current = false; setCloseDlg(null); }}>{tr("취소")}</button>
            </div>
          </div>
        </div>
      )}
          ...
          case "limb": {
            ...
            return (
              <g key={i} fill="none">{L(a.pts[0], a.pts[1], "#facc15")}{L(a.pts[2], a.pts[3], "#4ade80")}
                {T(X(a.pts[1]) + 5, Y(a.pts[1]), tr("L(좌)"), "#facc15")}
                {T(X(a.pts[3]) + 5, Y(a.pts[3]), tr("R(우)"), "#4ade80")}
          ...
          case "pelvis": {
            ...
            return (
              <g key={i} fill="none">{L(a.pts[0], a.pts[1], "#f97316")}
                ...
                {T(xR + 4, yRef - 4, `${tr("골반")} ${deg.toFixed(1)}° / Δ${Math.abs(dym).toFixed(1)}${unit}`, "#f97316")}</g>);
          }
          case "spineCurve": {
                ...
                {devIdx >= 0 && devFoot && (
                  <>
                    ...
                    {T(X(a.pts[devIdx]) + 7, Y(a.pts[devIdx]) - 6,
                       `${tr("척추 편위")} ${devMax.toFixed(1)}${unit}`, "#f87171")}
                  </>
                )}</g>);
          }
          case "ctr": {
